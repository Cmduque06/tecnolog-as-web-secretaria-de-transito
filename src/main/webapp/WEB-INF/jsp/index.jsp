<%@ page contentType="text/html;charset=UTF-8" %>
<!DOCTYPE html>
<html lang="es">
<head>
    <title>Secretaría de Tránsito</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body { background: linear-gradient(120deg, #eef4ff, #f7f9fc); }
        .list-group { border: 0; border-radius: 1rem; box-shadow: 0 8px 25px rgba(0,0,0,.08); }
    </style>
</head>
<body>
<div class="container mt-4">
    <h1>Secretaría de Tránsito</h1>
    <p>Sistema para gestión de propietarios, vehículos e infracciones.</p>
    <div class="list-group">
        <a class="list-group-item list-group-item-action" href="/propietarios">Propietarios</a>
        <a class="list-group-item list-group-item-action" href="/vehiculos">Vehículos</a>
        <a class="list-group-item list-group-item-action" href="/infracciones">Infracciones</a>
        <a class="list-group-item list-group-item-action" href="/reportes">Reportes</a>
    </div>
</div>
<script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
</body>
</html>
